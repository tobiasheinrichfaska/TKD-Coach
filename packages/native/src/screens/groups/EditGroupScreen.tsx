  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        <Text style={styles.title}>{groupId ? t('Edit group') : t('Create group')}</Text>
        <TextInput style={styles.input} placeholder={t('Group name')} placeholderTextColor={COLORS.textMuted} value={name} onChangeText={setName} />

        <Text style={styles.section}>{t('Training times')}</Text>
        {slots.map((slot, i) => (
          <View key={i} style={styles.slotCard}>
            ...
            <View style={styles.timeRow}>
              <View>
                <Text style={styles.fieldLabel}>{t('Start')}</Text>
                <TextInput style={styles.smallInput} value={slot.start} onChangeText={txt => updateSlot(i, { start: txt })} placeholder="17:00" placeholderTextColor={COLORS.textMuted} keyboardType="numbers-and-punctuation" maxLength={5} />
              </View>
              <View>
                <Text style={styles.fieldLabel}>{t('Minutes')}</Text>
                <TextInput style={styles.smallInput} value={String(slot.durationMin)} onChangeText={txt => updateSlot(i, { durationMin: Number(txt.replace(/\D/g, '')) || 0 })} keyboardType="number-pad" maxLength={3} />
              </View>
        ...
        <TouchableOpacity onPress={addSlot}>
          <Text style={styles.addLink}>+ {t('Add training time')}</Text>
        </TouchableOpacity>
